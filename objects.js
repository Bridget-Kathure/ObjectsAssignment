...
const people = [
  ...
  const peoples = people.filter(item=> {
    return (item.age >= 18)
  }
)
console.log(peoples)
...
const products = [
  ...
  const groupProducts = Object.groupBy(products, ({category}) => {
    return category
})
...
function studentsAverageScore(students) {
    return students.filter(student => {
       const newArray = student.scores.reduce((sum, score) => sum + score, 0) / student.scores.length;
       return newArray >= 85;
    }).map(student => student.name);
   }
   const studentsScore = studentsAverageScore(students);
   ...
   const car = {
    ...
    age: function() {
      ...
      const ageOfTheCar = currentYear - this.year;
      return ageOfTheCar;
    }
  };
  console.log(car.age());
